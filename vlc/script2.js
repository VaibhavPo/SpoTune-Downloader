...
function playSong(playlist, song) {
    ...
    howl = new Howl({
        ...
        onplay: () => updateDuration(),
        onend: () => nextSong(), // Auto-play next song
...
function nextSong() {
    ...
    playSong(currentPlaylist[currentIndex].playlist, currentPlaylist[currentIndex].song);
}
...
async function loadSongs(playlist) {
    const res = await fetch(`http://127.0.0.1:5000/playlist/${playlist}`);
    const songs = await res.json();
    currentPlaylist = songs.map(song => ({ playlist, song }));
    currentIndex = 0; // Reset to start
    // Render songs as before...
}
...
document.getElementById('shuffle-btn').onclick = shuffleSong;

document.getElementById('forward-btn').onclick = () => howl && howl.seek(howl.seek() + 10);
document.getElementById('backward-btn').onclick = () => howl && howl.seek(Math.max(howl.seek() - 10, 0));

document.getElementById('volume-slider').oninput = (e) => {
    Howler.volume(e.target.value); // Global volume
};
function updateDuration() {
    ...
    const updateInterval = setInterval(() => {
        if (howl.playing()) {
            ...
            timeline.value = Math.floor(seek);
        } else {
...
document.getElementById('timeline').oninput = (e) => {
    if (howl) howl.seek(e.target.value);
};
async function loadSongs(playlist) {
    const res = await fetch(`http://127.0.0.1:5000/playlist/${playlist}`);
    const songs = await res.json();
    ...
    if (currentPlaylist.length > 0) {
        playSong(currentPlaylist[0].playlist, currentPlaylist[0].song); // Autoplay first
    }
}
